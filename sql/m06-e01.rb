...
class MySQLExercises1
  ...
  def q_products_conditions
    ...
    <<-SQL
      SELECT usage_status FROM products GROUP BY usage_status
    SQL
  end
  ...
  def q_products_for_sale
    ...
    <<-SQL
      SELECT COUNT(id) AS num_for_sale FROM products WHERE original_price != price
    SQL
  end
  ...
  def q_products_discounts
    ...
    <<-SQL
      SELECT id, original_price, (1-price/original_price) AS discount FROM products
    SQL
  end
  ...
  def q_products_for_sale_half_price
    ...
    <<-SQL
      SELECT id, original_price, (1-price/original_price) AS discount FROM products HAVING discount > 0.5 ORDER BY discount DESC
    SQL
  end
  ...
  def q_sales_count_for_product
    ...
    <<-SQL
      SELECT title, sales.id AS sale_id 
      FROM   products 
      JOIN   sales ON products.seller_id = sales.seller_id 
      WHERE  title LIKE "Mochila Puma Training" AND sales.id != 402 
      ORDER BY sale_id
    SQL
  end
  ...
  def q_products_by_popularity
    ...
    <<-SQL
      SELECT    products.id, title, COUNT(sales.id) AS num_sales
      FROM      products
      JOIN      sales ON products.seller_id = sales.seller_id
      GROUP BY  products.id
      ORDER BY  num_sales DESC, title, id
    SQL
  end
  ...
  def q_products_without_category
    ...
    <<-SQL
      SELECT COUNT(*) AS uncategorized FROM products WHERE category_id IS NULL
    SQL
  end

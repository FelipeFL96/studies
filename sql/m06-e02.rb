...
class MySQLExercises2
  ...
  def q_categories_with_parent
    ...
    <<-SQL
      SELECT  par.name AS parent_name, chi.name AS category_name
      FROM    categories par 
      JOIN    categories chi ON par.id = chi.parent_category_id 
      ORDER BY par.name, chi.name
    SQL
  end
  ...
  def q_avg_stars_per_user
    ...
    <<-SQL
      SELECT users.id AS id, name, AVG(stars) AS avg_stars 
      FROM   reviews 
  ...
  def q_buyers_that_are_also_sellers
    ...
    <<-SQL
      SELECT sel.seller_id AS buyer_seller_id 
      FROM sales sel JOIN sales buy ON sel.seller_id = buy.buyer_id 
  ...
  def q_recently_sold_products_categories
    ...
    <<-SQL
      ...
      WHERE payment_method LIKE "Dinheiro" 
